		...
		func(tc test) {
			t.Run(tc.name, func(t *testing.T) {
				...
				for j := uint8(1); j <= extension.MaxSupportedFabridHops; j++ {
					...
					srcValidationNumber, srcValidationReply, err := crypto.InitValidators(f, id, s, tmpBuffer, pathKey, asHostKeys,
						asAsKeys, hops)
					assert.NoError(t, err)
					...
					dstValidationNumber, dstValidationReply, err := crypto.VerifyPathValidator(f, tmpBuffer, pathKey)
					assert.NoError(t, err)
