	...
	tests := []test{
		...
		},
		{
			...
			validate: func(fco *extension.FabridControlOption, err error, t *testing.T) {
				...
				assert.Equal(t, [4]byte{0x0F, 0x22, 0x33, 0x44}, fco.Auth, "Wrong auth")
				assert.Equal(t, uint32(0x07bbccdd), fco.Timestamp, "Wrong timestamp")
				assert.Equal(t, uint32(0xeeffaabb), fco.PacketID, "Wrong packetID")
				assert.Equal(t, []byte{0x11}, fco.Data, "Wrong data")
			},
		},
		{
	...
	tests := []test{
		...
		},
		{
			...
			validate: func(b []byte, err error, t *testing.T) {
				...
				assert.Equal(t, []byte{0x11}, b[12:13], "Wrong Data")
			},
		},
		{
